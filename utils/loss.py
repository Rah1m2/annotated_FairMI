...
def bpr_loss(user_emb, pos_emb, neg_emb):
    pos_score = torch.sum(user_emb * pos_emb, dim=1)  # 逐元素相承
    ...
    emb_loss = (1 / 2) * (user_emb.norm(2).pow(2) +
                          ...
                          neg_emb.norm(2).pow(2)) / user_emb.shape[0]  # 1/2M
...
def condition_info_nce_for_embeddings(x, z, s, p, alpha=0.1, tau=0.5):
    ...
    x_norm = F.normalize(x)  # 预训练好的模型Pu
    y_norm = F.normalize(z + alpha * s)  #

    pos_score = torch.sum(x_norm * y_norm, dim=1)  # 分子
    ...
    cl_loss = -torch.log(pos_score / neg_score).mean()  # 计算loss就需加-号，因为原本目标是最大化下界
    return cl_loss
