...
def train_semigcn(gcn, sens, n_users, lr=0.001, num_epochs=1000, device='cpu'):
    ...
    for _ in tqdm(range(num_epochs)):
        _, _, su, _ = gcn()  # 保留用户嵌入预测出来的结果，其他的都丢掉
        shuffle_idx = torch.randperm(n_users)
        classify_loss = F.cross_entropy(su[shuffle_idx].squeeze(), sens[shuffle_idx].squeeze())  # 参数1：数据；参数2：分类标签
        optimizer.zero_grad()
...
def train_unify_mi(sens_enc, inter_enc, club, dataset, u_sens,
    ...
    e_su = e_su.detach().to(args.device)  # 分离出来，不计算梯度
    e_si = e_si.detach().to(args.device)
    p_su = conditional_samples(e_su.detach().cpu().numpy())  # 将元素分为两类
    ...
    for epoch in range(args.num_epochs):
        ...
        for uij in train_loader:
            ...
            lb1 = condition_info_nce_for_embeddings(e_xu[torch.unique(u)], e_zu[torch.unique(u)],
                                                   e_su[torch.unique(u)], p_su[torch.unique(u)])  # dataloader抽出的2048个数据是随机抽出的，可能有重复
...
if __name__ == '__main__':
    ...
    norm_adj = graph.generate_ori_norm_adj()  # 邻接矩阵
    ...
    # 用lgc作为兴趣编码器的backbone，bpr模型用的bprmf
    ...
    train_semigcn(sens_enc, u_sens, n_users, device=args.device)  # 首先训练出针对用户bias进行分类的模型
